...
class Weekday(Enum):
    ...
    def from_str(string: str) -> "Weekday":
        ...
        """
        return util.get_unique_enum_prefix_match(string, Weekday, value_name="weekday")
...
@dataclass
class TimeDay:
    """Time with a weekday (not a specific date)."""
    ...
    def __str__(self):
        return (
            f"{self.day.name.title()[:3] + ' @ ' if self.day else ''}"
...
class TimeRange:
    ...
    def __post_init__(self):
        if (self.start.day is None) != (self.end.day is None):
            raise ValueError(
                "Constraint time range: either *both* the start and end should have a "
                f"weekday, or *neither* should have a weekday, but got {self}."
            )
    ...
    def next_change_time(self, now_dt: dt) -> dt:
        ...
        use_time_only: bool = (
            candidate_timeday.day is None
            or Weekday.from_dt(now_dt) == candidate_timeday.day
        )
...
def within_constraints(
    now: TimeDay | None, ranges: list[TimeRange] | TimeRange
) -> bool:
    """
