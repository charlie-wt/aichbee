...
def get_filename(allow_nonroot_fallback: bool = False) -> Path:
    ...
    :param allow_nonroot_fallback: If True, and this function is being run by a non-root
                                   ...
                                   ``$XDG_CONFIG_HOME``; otherwise, will always return
                                   the system location.
    ...
    for line in data:
        ...
        if line[0] == "=":
            ...
            if in_a_group:
                ...
                in_a_group = True
                blockgroups.append(
                    BlockGroup(parse.parse_name(line), config_path=Path(filename))
                )
            ...
            if line[0] == "@":
                ...
                if not current_group.schedule_constraints_consistent():
                    raise ValueError(
                        f"time constraints on block group {n.display_name()} overlap, "
