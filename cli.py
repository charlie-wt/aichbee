...
def mark_could_not_read_runtime_info(group: BlockGroup) -> None:
    """Mark that we could not read runtime info from the running service for this group.
    If done, later calls to ``could_read_runtime_info`` on ``group`` will return
    ``False``.
...
def send_message(*args: str) -> None:
    """Send a message to the running service. Will error if the service isn't
    running.
    """
...
def show(
    group_name: str, bf_path: Path | None = None, should_colour: bool = True
) -> None:
    """
...
def set_paused(group_name: str, paused: bool, bf_path: Path | None = None) -> None:
    """Either pause or unpause a block group, if it's one with a duration-based
    block.
    """
    ...
    if to_pause.duration is None:
        logging.warning(
            "Can only pause block groups with duration constraints, but "
...
def main():
    ...
    common_args_parser.add_argument(
        ...
        help=(
            "Should output be coloured? If not set, will choose based on whether "
    ...
    parser_list = subparsers.add_parser(
        "list", parents=[common_args_parser], help="List block groups"
    )
    ...
    parser_show.add_argument(
        "show_block_group", help="Name of the block group to show."
    )
    ...
    parser_pause.add_argument(
        "pause_block_group", help="Name of the block group to pause."
    )
    ...
    parser_unpause.add_argument(
        "unpause_block_group", help="Name of the block group to unpause."
    )
    ...
    if args.command is None:
        ...
        print("Please provide a command, or -h/--help to print help.", file=sys.stderr)
        return
    ...
    try:
        ...
        logging.error(
            f"{colour.red('ERROR')}: connection to service refused. Are you sure the "
            "service is running?"
        )
        sys.exit(1)
