    ...
    def next_reset_at(self, prev_reset: dt) -> dt:
        ...
        match self:
            ...
            case DurationPeriod.EACH_MONTH:
                return midnight(
                    (prev_reset.replace(day=1) + timedelta(days=32))
                    .replace(day=1)
                    .date()
                )
...
class BlockGroup:
    ...
    def state_path(self) -> Path:
        ...
        if self.name is None:
            raise ValueError(
                "Tried to get the state path of an unnamed block group. Any block "
                "group with a duration-based constraint (and thus needing to save "
    ...
    def update_state(self, now: dt | None = None) -> None:
        ...
        if self.is_paused:
            ...
            if (
                not self.within_schedule_constraints(now)
                and self.prev_time_spent_paused_update is not None
            ):
    ...
    def schedule_constraints_consistent(self) -> bool:
            ...
            for i in range(len(ranges)):
                for j in range(len(ranges)):
                    ...
                    if bad_start or bad_end:
                        logging.debug(
                            f"constraints for group {self.display_name()}: ",
                            f"{ranges[i]} vs. {ranges[j]}: bad start: {bad_start}, bad "
                            f"end: {bad_end}",
                        )
                        return False
            return True
        ...
        return ranges_consistent(time_only_ranges) and ranges_consistent(
            day_based_ranges
        )
    ...
    def __str__(self) -> str:
        ...
        if (
            len(sites_to_list) > max_sites_to_list
            and logging.root.level >= logging.DEBUG
        ):
            sites_to_list = self.sites[: max_sites_to_list // 2]
            sites_to_list.append(
                f"... {len(self.sites) - max_sites_to_list} more sites ..."
            )
